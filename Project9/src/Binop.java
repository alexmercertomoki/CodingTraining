...
public abstract class Binop extends Node {
	...
	*/
    ...
    /** Find the parent and child nodes for crossover.
    There are three NodePairPlus objects; exactly one of them will be
    returned, depending on whether the cuurrent node is the parent,
    or the parent is in the left subtree, or the parent is in the
    right subtree.
    clipNumber is the pre-determined location of the clip point.
    nodeNumber is the number of the current node.  If
    nodeNumber + 1 == clipNumber, the current node is the parent,
    and the left child is the child. Otherwise, send testLeft down
    the left subtree.  If it comes back instantiated, i.e.,
    testLeft.parent != null, then the clip point was found down
    the left subtree.  Return testLeft.  If testLeft's counter
    is one less than the clip number, the current node is the parent
    and the right child is the child.  Otherwise, search for the clip
    point down the right subtree.
	*/
    ...
    /** Exchange a child tree with a different child tree (one
    half of the crossover operation).
	Test to see which of lChild or rChild is the old node, and replace
